...
const verifyToken = async (token: string): Promise<boolean> => {
  try {
    // Log environment variables used for verification
    console.log("Verifying JWT with Client ID:", process.env.CANVA_CLIENT_ID);
    console.log("Using JWKS URL base:", process.env.BASE_CANVA_CONNECT_API_URL);

    const publicKeysURL = new URL(
      "rest/v1/connect/keys",
      process.env.BASE_CANVA_CONNECT_API_URL,
    );
    console.log("Constructed JWKS URL:", publicKeysURL.toString());
    ...
    console.log("JWT Verified Successfully. Header:", protectedHeader);
    return !!protectedHeader;
  } catch (error) {
    // Log the specific error during verification
