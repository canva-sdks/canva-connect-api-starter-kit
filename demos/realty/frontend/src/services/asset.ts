...
export const uploadAsset = async ({
  ...
  if (!asset) {
    throw new Error(
      `Asset upload for "${name}" with job id "${result.job.id}": ${result.job.error?.code} ${result.job.error?.message ?? "Unknown error"}`,
    );
  }
...
const pollAssetUpload = async (jobId: string): Promise<Asset> => {
  ...
  if (!response.job.asset) {
    throw new Error(
      `Asset upload for job id "${response.job.id}" was unsuccessful: ${response.job.error?.code} ${response.job.error?.message ?? "Unknown error"}`,
    );
  }
